...
use App\Models\Event as AcaraEvent;
...
class Event extends Component
{
    ...
    public function render()
    {
        ...
        // $Acara = AcaraEvent::when($this->cariKategori,function($q){
        //     $q->where('kategorievent_id', $this->cariKategori);
        // })->search(trim($this->search))->paginate(10);
        // $Acara =  $this->search === null ? AcaraEvent::latest()->paginate(10)
        // : AcaraEvent::where('nama_event', 'LIKE' , '%'.$this->search.'%')
        // ->when($this->cariKategori,function($query){
        //     $query->where('kategori_event',$this->cariKategori);
        // })

        // ->orWhere('pendaftaran', 'LIKE' , '%'. $this->search.'%')->latest()->paginate(5);
        // $Acara = AcaraEvent::when($this->cariKategori,function($query){
        //     $query->where('kategorievent_id',$this->cariKategori);
        // });
        return view('livewire.Event.event',[
            'Kategori_event' => $kategori_event,

            'Acara' => $this->search === null ? AcaraEvent::latest()->paginate($this->perhalaman) :
            AcaraEvent::where('nama_event' , 'LIKE' , '%'.$this->search.'%')->latest()
            ->when($this->cariKategori,function($query){
                $query->where('kategorievent_id',$this->cariKategori);
    ...
    public function simpan() {
        ...
        AcaraEvent::updateOrCreate(['id' => $this->event_id],[
    ...
    public function edit($id){
        $acara = AcaraEvent::findorFail($id);
        $this->event_id = $id;
