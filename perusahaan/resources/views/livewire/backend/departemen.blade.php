...
<div class=" max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -4 m-3">
    <div class="flex">
        <div class="mt-5 md:mt-0 md:col-span-2 w-full">
            ...
            @if ($isModalOpen)
            @include('livewire.modal.modal-departement')
            @endif
                            ...
                            <table class="table-auto w-full text-left whitespace-no-wrap">
                                ...
                                <tbody>
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td class="px-2 py-3 text-sm">{{ $departement->nama_departement }}</td>
                                        <td class="px-2 py-3 text-sm text-gray-900">
                                            {{-- edit --}}
                                            <button wire:click="edit({{ $departement->id }})" class="bg-yellow-500 mx-2 px-2 py-1 text-white">
                                            ...
                                            @if($konfirmasi === $departement->id)
                                            <button wire:click="hapus({{ $departement->id }})" class="bg-red-500 px-2 py-1 text-white">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            @else
                                            {{-- konfirmasi hapus --}}
                                            <button wire:click="konfirmasiHapusId({{ $departement->id }})" class="bg-red-500 text-white px-2 py-1">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
