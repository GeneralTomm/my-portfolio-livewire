            ...
            <div class="bg-white shadow-md px-2 py-2 my-2">
                <div class="flex justify-end w-auto mt-5 px-5">
                    <input wire:model="search" type="search"
                        ...
                        Cari Data">
                    <select wire:model="cariKategori"
                        class="block appearance-none bg-white border border-gray-200 text-gray-700 py-1 px-2 pr-8 mx-2 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                        id="grid-state">
                    ...
                    <select wire:model="cariPendaftaran"
                        class="block appearance-none bg-white border border-gray-200 text-gray-700 py-1 px-2 pr-8 mx-2 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                        id="grid-state">
                                ...
                                <tbody>
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td class="px-2 py-3 text-sm text-gray-900">
                                            {{ date('d', strtotime($acara->acara_mulai)) }} -
                                            {{ date('d M Y', strtotime($acara->acara_selesai)) }}
                                        </td>
                                        <td class="px-2 py-3 text-sm font-bold">
                                            {{ $acara->kategorievent->nama_kategori }}</td>
                                        <td class="px-2 py-3 text-sm text-gray-900">
                                        ...
                                        @endif
                                        <td class="px-2 py-3 text-sm text-gray-900">
                                            <button wire:click="edit({{ $acara->id }})"
                                                class="bg-green-100 mx-2 px-2 py-1">Edit</button>
                                            <button wire:click="hapus({{ $acara->id }})"
